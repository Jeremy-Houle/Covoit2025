...
window.openDetailsIds = window.openDetailsIds || new Set();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (originalToggleDetails) {
        window.toggleDetails = function(idTrajet) {
            const result = originalToggleDetails.apply(this, arguments);
            const detailsDiv = document.getElementById('details-' + idTrajet);
            if (detailsDiv) {
                if (detailsDiv.style.display === 'block') {
                    window.openDetailsIds.add(String(idTrajet));
                } else {
                    window.openDetailsIds.delete(String(idTrajet));
                }
            }
            return result;
        };
    ...
    async function handleDetailsClick(detailsBtn) {
        ...
        let detailsPanel = document.getElementById(`details-${id}`);
        if (!detailsPanel) {
            detailsPanel = card.querySelector(`.trajet-details-${id}`);
        }
        if (!detailsPanel) {
            detailsPanel = card.querySelector('.trajet-details');
        }
        ...
        if (detailsPanel) {
            ...
            const btn = card.querySelector(`.btn-details-${id}`) || detailsBtn;
            if (btn) {
                btn.textContent = isHidden ? 'Masquer' : 'Détails';
            }
    ...
    function handleTrajetClick(ev) {
        ...
        if (detailsBtn && !detailsBtn.hasAttribute('onclick')) {
